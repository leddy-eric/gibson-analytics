...
public class ExampleGameStatsProvider extends AbstractMlbGameStatsProvider {
	...
	public GameStatistic createStatistics(Game game, List<Player> homeRoster, List<Player> awayRoster) {
		Map<Integer, Map<String, BigDecimal>> homePlayerMap = extractStatisticsByPlayer(homeRoster);
		// Map<Integer, Map<String, BigDecimal>> awayPlayerMap = extractStatisticsByPlayer(awayRoster);
		
		// First batters stats
		Map<String, BigDecimal> firstBatter = homePlayerMap.get(new Integer(1));
